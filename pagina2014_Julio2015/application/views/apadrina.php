	   	...
	   	function paso2(){
			if (($this->session->userdata("is_login") == FALSE)){
				...
				<script>
		   		...
		   		if($_POST == NULL){
		   			$data['ninoEspecial'] =$this->apadrina_modelo->traePrimerNino();
		   		}else{
		   			$data['ninoEspecial'] =$this->apadrina_modelo->traeNinoEspecial($_POST['ninoEspecial']);
		   		...
		   		$this->load->view("selecciona_nino",$data);
		   	}
	   	}
...
/*	   	function siguiente_nino_ajax($idnino){
	   		$sig = $this->apadrina_modelo->traeSiguiente($idnino);
	   		echo json_encode($sig);
	   	}

	   	function anterior_nino_ajax($idnino){
	   		$sig = $this->apadrina_modelo->traeAnterior($idnino);
	   		echo json_encode($sig);
	   	}*/
